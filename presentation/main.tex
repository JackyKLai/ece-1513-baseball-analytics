...
\vspace{-2pt}
{\scriptsize\color{gray}%
Regression ~$\cdot$~ Best alpha: 88.6}
...
% ── LEFT COLUMN ──
\begin{column}{0.495\textwidth}
{\scriptsize\textbf{Alpha Selection Curve}}

\vspace{0pt}
\includegraphics[width=\textwidth,height=0.40\textheight,keepaspectratio]{../results/ridge/graph2.png}
...
\centering
{\tiny\color{gray}MAE}\\[0pt]
{\small\textbf{\color{teal}8.47}}\\[-1pt]
{\scriptsize\color{gray}vs 10.73}
\end{column}
\begin{column}{0.45\linewidth}
\centering
{\tiny\color{gray}RMSE}\\[0pt]
{\small\textbf{\color{teal}10.95}}\\[-1pt]
{\scriptsize\color{gray}vs 13.48}
\end{column}
\end{columns}
\vspace{-1pt}
...
\begin{columns}[c]
\begin{column}{0.69\linewidth}
\includegraphics[width=\textwidth,height=0.44\textheight,keepaspectratio]{../results/ridge/graph1.png}
\end{column}
\begin{column}{0.29\linewidth}
\centering
\colorbox{teal!10}{%
...
\hline
\textbf{Model} & \textbf{MAE} & \textbf{RMSE} & \textbf{Params} & \textbf{MAE $\Delta$} \\
\hline
Pythagorean       & 10.73 & 13.48 & 0 (formula)       & ---             \\
\textbf{Ridge} ($\alpha\!=\!88.6$)  & \textbf{\color{teal}8.47}  & \textbf{\color{teal}10.95} & 15  & $-$21\%  \\
SVR (linear, $C\!=\!0.1$)  & {\color{teal}8.52}  & {\color{teal}10.98} & \#SVs  & $-$21\%  \\
Neural Net        & 9.85  & 12.59 & 3{,}137  & $-$8\%   \\
\hline
\end{tabular}}
\end{center}
...
{\scriptsize\textbf{Why do simpler models win?}}

\vspace{2pt}
{\scriptsize
\begin{itemize}
  \setlength{\itemsep}{2pt}
  \item \textbf{Limited data} (${\sim}$570 train rows):
    more parameters $\Rightarrow$ higher memorization risk
    {\tiny\color{gray}}
  \item \textbf{Bias--variance trade-off:}
    Ridge adds small bias, greatly reduces variance
    {\tiny\color{gray}}
  \item \textbf{SVR best kernel = linear} $\Rightarrow$ the
    runs$\to$wins relationship is approximately linear;
    RBF adds capacity the data doesn't need
    {\tiny\color{gray}}
  \item \textbf{NN:} universal approximation guarantees capacity
    {\tiny\color{gray}}, but SGD on small data
...
\begin{block}{\scriptsize Key Takeaways}
\vspace{-2pt}
{\scriptsize
\begin{enumerate}
  \setlength{\itemsep}{2pt}
  \item All ML models beat the no-learning baseline
  \item \textbf{$\ell_2$ regularization}
    {\tiny\color{gray}} is the single
    most effective technique on small data
  \item Model complexity must match data size ---
    more params $\neq$ better
    {\tiny\color{gray}}
  \item Season-to-season randomness (injuries, trades)
    sets a floor $\approx$\,Bayes error
    {\tiny\color{gray}}
\end{enumerate}
}
\vspace{-2pt}
